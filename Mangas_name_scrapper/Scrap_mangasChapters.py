...
for manga_name in datas['name']:
    ...
    try:
        ...
        try:
            element_0 = driver.find_element_by_xpath(balise_0).text # valeur qui indique si c'est un webtoon
        except:
            element_0 = driver.find_element_by_xpath(balise_00).text # valeur qui indique si c'est un webtoon ( option 2 )

        element_000 = driver.find_element_by_xpath(balise_000).text # valeur qui indique la date de dernière publication d'un chapitre/volume du manga
        ...
        if element_0.lower() == 'webtoon' or last_update <= 2020: # Si le manga est un webtoon ou sa dernière update remonte à 2020 ou moins => suppression
            ...
            while True:
                try:
                    ...
                    element_1 = driver.find_element_by_xpath(balise_1) # Chemin vers la balise contenant le chapitre {i} ou le volume {i} disponible
                    while True:
                        try:
                            balise_1 = str(f'//*[@id="collapse-{i_2}"]/div[{i}]/a')
                            element_1 = driver.find_element_by_xpath(balise_1) 
